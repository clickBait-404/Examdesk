...
export default function CreateExamPage() {
        ...
        try {
          ...
          const questions = rows
            ...
            .map((row) => ({
              ...
              options: [
                { text: row.option_a?.trim(), is_correct: row.correct?.trim().toUpperCase() === 'A' },
                { text: row.option_b?.trim(), is_correct: row.correct?.trim().toUpperCase() === 'B' },
                { text: row.option_c?.trim(), is_correct: row.correct?.trim().toUpperCase() === 'C' },
                { text: row.option_d?.trim(), is_correct: row.correct?.trim().toUpperCase() === 'D' },
          ...
          if (!response.ids || response.ids.length === 0) {
            toast.error('Questions were uploaded, but their IDs were not returned by the server')
            return
          }

          setSelectedQuestionIds((prev) => [...prev, ...response.ids])

          qc.invalidateQueries({ queryKey: ['exam-questions'] })
          qc.invalidateQueries({ queryKey: ['questions'] })

          toast.success(`${response.created} questions uploaded and selected`)
        } catch (error: any) {
          console.error('Question upload failed:', error)
          const message = error?.response?.data?.detail || error?.message || 'Failed to upload questions'
          toast.error(message)
        } finally {
  ...
  /*
   * Distribute total_marks evenly across selected questions.
   * Rounds to 2 decimal places and pushes any leftover remainder
   * onto the last question so the sum always equals total_marks exactly.
   */
  function distributeMarks(totalMarks: number, questionCount: number): number[] {
    if (questionCount === 0) return []

    const base = Math.floor((totalMarks / questionCount) * 100) / 100
    const marks = new Array(questionCount).fill(base)
    const distributed = base * questionCount
    const remainder = Math.round((totalMarks - distributed) * 100) / 100

    marks[marks.length - 1] = Math.round((marks[marks.length - 1] + remainder) * 100) / 100
    return marks
  }
  ...
  const publishMutation = useMutation({
    ...
    onError: (error: any) => {
      const message = error?.response?.data?.detail || error?.message || 'Failed to publish exam'
      toast.error(message)
    },
  })
  ...
  async function onFinalSubmit(data: FormData, publish: boolean) {
    try {
      ...
      const message = error?.response?.data?.detail || error?.message || 'Failed to save/publish exam'
      toast.error(message)
    }
  }
        ...
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center flex-1">
            <div className="flex flex-col items-center cursor-pointer" onClick={() => step > i && setStep(i)}>
              ...
              <span className={clsx('text-xs mt-1 font-medium', step === i ? 'text-brand-600' : 'text-gray-400')}>
                {s}
              </span>
            </div>
            {i < STEPS.length - 1 && (
      ...
      <Card className="max-w-3xl">
        ...
        {step === 1 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-base font-semibold text-gray-800">Select Questions</h3>
            ...
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
              <p className="text-sm text-blue-700">
                <strong>{selectedQuestionIds.length}</strong> question(s) selected
              </p>
              <p className="text-xs text-blue-600 mt-1">
                CSV columns: text, question_type, difficulty, topic, option_a, option_b, option_c, option_d, correct, explanation
              </p>
            </div>

            {uploadingQuestions && <div className="mb-4 text-sm text-gray-500">Uploading questions...</div>}

            {questionsLoading ? (
              <div className="py-8 text-center text-sm text-gray-500">Loading questions...</div>
            ) : (
              <div className="space-y-2 max-h-[420px] overflow-y-auto">
                {(questionData?.items || []).map((question: Question) => {
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex gap-2 mb-1">
                            ...
                            {question.topic && <span className="text-xs text-gray-500">• {question.topic}</span>}
                          </div>
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-8 text-sm">
              {(
                [
                  ...
                  ['Duration', `${watchData.duration_minutes ?? 0} minutes`],
        ...
        <div className="flex items-center justify-between mt-8 pt-5 border-t border-gray-100">
          <Button
            type="button"
            variant="secondary"
            onClick={() => (step > 0 ? setStep((s) => s - 1) : navigate('/exams'))}
            disabled={step === 0}
          >
            {step === 0 ? 'Cancel' : '← Back'}
          </Button>
